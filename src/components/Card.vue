<template>
  <v-hover v-slot="{ hover }">
    <v-card
      :loading="loading"
      class="mx-auto my-12 rounded-xl card-comp"
    ...
    >
      <v-img height="250" :src="person.Photo" aspect-ratio="1" class="image">
        ...
        <v-row
          align="end"
          justify="start"
          class="fill-height mt-1 d-flex flex-column"
        >
          <v-col
            cols="auto"
            class="pa-1 pr-3"
      ...
      <v-card-text>
        <div class="info-block">
          ...
          <v-progress-linear
            ...
            :color="
              person.Profit[0].Color ? `#${person.Profit[0].Color}` : black
            "
        ...
        <div class="info-block">
          <h4 class="heading">
            Attention <span class="heading__span">48 h</span>
          </h4>
          <v-row
            no-gutters
            v-if="person.Attention && person.Attention.length > 0"
          >
        ...
        <div class="info-block">
          <v-row dense v-if="person.Attention && person.Attention.length > 0">
            <v-col
              cols="6"
              v-for="(item, index) in person.Attention"
              :key="index"
              class="rounded-lg"
            >
              <v-progress-linear
          ...
          <v-row dense v-else>
            <v-col cols="12" class="rounded-lg">
              <v-progress-linear height="25" color="grey" :value="0" rounded>
                <template v-slot:default="{ value }">
                  <strong style="color: white">{{ Math.ceil(value) }}%</strong>
                </template>
              </v-progress-linear>
